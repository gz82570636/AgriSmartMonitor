...
int cgiMain()
{
	...
	if((shmid = shmget(key, N, IPC_CREAT | 0666 )) == -1)//创建共享内存
	{
	...
	if (shm_buf->shm_status == 1)
	{
		...
		fprintf(cgiOut, "<h4>TemMAX:\t%0.2f°CRH</H4>\n ", shm_buf->sm_all_env_info.monitor_no[shm_buf->shm_status].zigbee_info.tempMAX);
		fprintf(cgiOut, "<h4>TemMIN:\t%0.2f°CRH</H4>\n ", shm_buf->sm_all_env_info.monitor_no[shm_buf->shm_status].zigbee_info.tempMIN);
		fprintf(cgiOut, "<h4>Humidity:\t%0.2f O/oRH</H4>\n ", shm_buf->sm_all_env_info.monitor_no[shm_buf->shm_status].zigbee_info.humidity);
		fprintf(cgiOut, "<h4>HumidityMAX:\t%0.2f O/oRH</H4>\n ", shm_buf->sm_all_env_info.monitor_no[shm_buf->shm_status].zigbee_info.humidityMAX);
		fprintf(cgiOut, "<h4>HumidityMIN:\t%0.2f O/oRH</H4>\n ", shm_buf->sm_all_env_info.monitor_no[shm_buf->shm_status].zigbee_info.humidityMIN);
